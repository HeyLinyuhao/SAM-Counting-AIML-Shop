...
class SamAutomaticMaskGenerator:
    ...
    @torch.no_grad()
    def generate(self, image: np.ndarray, ref_bbox,SLIC_centers) -> List[Dict[str, Any]]:
        """
        ...
        mask_data, mask_data2 = self._generate_masks(image, ref_bbox,SLIC_centers)
    ...
    def _generate_masks(self, image: np.ndarray, ref_box,SLIC_centers) -> MaskData:
        ...
        for crop_box, layer_idx in zip(crop_boxes, layer_idxs):
            # print(layer_idx)
            crop_data = self._process_crop(image, crop_box, layer_idx, orig_size, ref_box,SLIC_centers)
